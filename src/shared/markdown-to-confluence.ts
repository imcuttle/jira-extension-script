...
var langArr = 'actionscript3 bash csharp coldfusion cpp css delphi diff erlang groovy java javafx javascript perl php none powershell python ruby scala sql vb html/xml'.split(
  /\s+/
)
var langMap = {
  shell: 'bash',
...
_.extend(Renderer.prototype, rawRenderer.prototype, {
  ...
  list: function (body, ordered) {
    ...
    return (
      _.map(arr, function (line, i) {
        if (/^([#*]+)(\s[^]*)$/.test(line)) {
          return type + RegExp.$1 + RegExp.$2
        }
        return type + ' ' + line
      }).join('\n') + '\n\n'
    )
